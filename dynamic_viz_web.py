...
def run_simulation(network: DynamicNetwork) -> list[dict]:
    ...
    # Преобразуем все словари в объекты dataclass
    for mid, m in list(network.movements.items()):
        if isinstance(m, dict):
            to_link = network.links.get(m['to_link_id'])
            node_id = to_link.start_node_id if to_link else ''
            m['node_id'] = node_id
            network.movements[mid] = Movement(**m)
    
    for sid, s in list(network.sources.items()):
        if isinstance(s, dict):
            network.sources[sid] = Source(**s)
    
    for sid, s in list(network.sinks.items()):
        if isinstance(s, dict):
            network.sinks[sid] = Sink(**s)
